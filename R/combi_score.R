...
#'@description A function that applies the previously calculated models to an unclassified dataset to compute combi scores and optionally classify the samples.
#'@details This function  can take as input datasets loaded with load_unclassified_data(). They MUST contain all the markers of the classified dataset used to train the models (the one loaded with load_data()).
...
#' @export
  ...
  combi_score <- function(unclassified_data, Models, Metrics, Positive_class=1, Negative_class=0, deal_NA='impute', classify=F){
