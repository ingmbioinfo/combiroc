...
head(rmks$table)
```
And with `roc_reports()` we carry out the computations for the four top selected combinations (we need to explicitly indicate their number IDs in the `selected_combinations` argument). Also, we need to indicate the case class (NK):
```{r}
...
The CBMC test dataset was directly downloaded from [here](ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE100nnn/GSE100866/suppl/GSE100866_CBMC_8K_13AB_10X-RNA_umi.csv.gz) as described in the [Seurat multimodal vignette](https://satijalab.org/seurat/articles/multimodal_vignette.html)
...
DimPlot(cbmc, repel = T, label = T) + NoLegend()
```
