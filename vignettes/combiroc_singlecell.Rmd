...
In this context we decided to employ combiroc in the scRNA-seq analysis field as a *signature refiner*, in order to find the best **few** marker genes combinations from a list of differentially expressed genes, *regardless their ranking* as differentially expressed gene. Being combiroc specialized in finding specific combinations, its use may allow researchers to find some cells of interest with a high degree of confidence and help them in the process of manual annotation by reducing the number of marker genes to consider.
...
To be able to run the whole workflow without high memory or computational requirements the user needs to __download the pre-computed files__, preserving the file names and folder structure (or changing the code accordingly).
...
The difficulty to distinguish NK (Natural Killer) cells from CD8+ T cells in many scRNAseq datasets is a good challenge to test combiroc performance in markers signature optimization; after performing differential marker expression we took into account the top 30 differentially expressed genes (DEGs) specific for NK cells cluster. We are going to use combiroc to select the best combinations of markers among these top 30 NK cells markers. Regression models from the selected combinations will then be used on other independent datasets of the same kind (test/validation datasets). 

__TRAINING DATASET__  

As combiroc training dataset we used the [PBMC CITE-seq atlas](https://satijalab.org/seurat/articles/multimodal_reference_mapping.html) from Hao et al. 2021 [[2]](https://www.sciencedirect.com/science/article/pii/S0092867421005833). This dataset can be downloaded as an h5 Seurat data (h5s) from the [FredHutch-NYCG atlas page](https://atlas.fredhutch.org/nygc/multimodal-pbmc/).
...
To select the annotations *(Code not run)*:
```{r warning=FALSE, message=FALSE}
atlas <- readRDS('data/pbmc_multimodal.rds')
atlas
```
...
To perform differential expression analysis *(Code not run)*:
```{r eval=FALSE}
...
The suggested threshold is indicated by the vertical dashed line and its value is shown in overlay. The exact value can be retrieved from the `distr` object by `distr$Coord$threshold`:
```{r}
autoThreshold <- distr$Coord$threshold
```
...
```{r warning=F}
reports$Plot
rocplot <- (reports$Plot) + xlim(0, 0.20)
rocplot
reports$Metrics
```
...
- Cord Blood mononuclear cells (CBMC) CITE-seq dataset shown in ['Using Seurat with multimodal data'](https://satijalab.org/seurat/articles/multimodal_vignette.html) vignette 
- Plasma Blood mononuclear cells (PBMC) [PBMC3K scRNA-seq](https://satijalab.org/seurat/articles/pbmc3k_tutorial.html) dataset from Satija, R., Farrell, J., Gennert, D. et al. 2015 [[3]](https://www.nature.com/articles/nbt.3192)
...
The CBMC test dataset, made of 8617 cord blood mononuclear cells (CBMCs), produced with CITE-seq was directly loaded from the `SeuratData` package. The dataset hah been generated by Stoeckius et al. 2017 [[4]](https://www.nature.com/articles/nmeth.4380). Further description can be found in the [Seurat multimodal vignette](https://satijalab.org/seurat/articles/multimodal_vignette.html) or by calling the guide with `?cbmc`.
...
# remove "mouse" cells (tagged with NAs)
cbmc[['ID']]<- Idents(cbmc)
# cbmc <- cbmc[, cbmc[['ID']]!="NA"] # keep the mouse cells!!!
# visualize UMAP clusters
DimPlot(cbmc, repel = T, label = T)
```
Please note that the "NA" gray cell scluster is from mouse cells whose presence acts as negative control in the original experiment/dataset.


Using `seurat_to_combiroc()` we now obtain the first combiroc test dataset (`test_cbmc`, containing cell barcodeds as sample IDs and the expression values of the 30 NK signature genes).
```{r, message=FALSE, warning=FALSE}
...
The PBMC3K test dataset can also be directly invoked from the `SeuratData` library. The dataset is immediately available as a Seurat object named `pbmc3k.final`
```{r, message=FALSE, warning=FALSE}
#InstallData("pbmc3k")
data("pbmc3k")
...
```
In the same way as oreviously done we are using now `seurat_to_combiroc()` to obtain the second combiroc test dataset (`test_pbmc3k`). Please note that this dataset does not contain 'TRDC expression values since this gene ' (which is part of the selected combinations) is not present in the original PBMC3K dataset.
```{r, message=FALSE, warning=FALSE}
...
p1 | p2
```
In CBMC dataset, single markers values are higher in NK cluster and moderate in all the others, but still slightly present in some CD8 T cells, CD34+ and CD16+ Mono cells, even if clusters have been defined by the authors relying on protein-level (CITE seq) information. Moreover, GZMB per se is not sufficient to discriminate NK cells from pDC. In this dataset only PRF1 among the top four markers is able to discriminate NK cells alone. Mouse cells (the "NA" cluster), which are kept from the original dataset as negative control, also show a bit of GNLY and NKG7 expression.
...
We now look at the performance of the selected marker combinations. To assess the descriptive power, for both test datasets, we compare the expression of top 4 single markers with the expression of the best combination (since the top 4 are very similar in terms of performance), exploiting both our "combi-score" and an already published "gene-signature-score".

While the __combi-score__ defined here is defined as the predicted probability obtained while fitting the combinations models to the test datasets (returned by `combi_score()`), the __gene-signature-score__ that we use as a comparison is described in Della Chiara et al. 2021 [[5]](https://www.nature.com/articles/s41467-021-22544-y): it takes into account both the expression level and co-expression of genes within each single cell. Given a geneset, the gene-signature-score's increase is directly proportional to the number of expressed genes in the signature and to the sum of their level of expression. We reproduced this score in the custom R function `signature_score.R`:
```{r}
source("inst/external_code/signature_score.R")
```

First, we compute the "gene-signature-score" of the __whole 30 DEGs signature__ that we assume as reference.
```{r}
...
We then calculate both the "gene-signature-score" and the "combi-score" of the top combinations:
- _Combination 172173_ (made by genes GZMB, IL2RB, KLRF1, SPON2 and TRDC), with the highest accuracy, is investigated in CBMC dataset.
- _Combination 137550_ (made by genes CLIC3, FCGR3A, IL2RB, KLRD1 and KLRF1) is used in PBMC3K dataset since it's the first top combination without the TRDC gene, which is not expressed in the this dataset.
```{r}
...
Strikingly, in CBMC the highest scores are all found in NK cluster, while in all the others the values are drastically lower and all comparable to mouse cell ones (negative control). In PBMC3K the highest scores are in NK cells but a still relevant score is found in cytotoxic CD8-T cells. The signal of the whole-30genes-signature is, as expected, less noisy than the signal from the single top markers.
...
Remarkably, despite the 6 fold reduction in the number of considered features to build the gene score,  for both the datasets, the signature score confirmed that the best combination is, at worst, as accurate as the whole signature in indicating the NK cluster. In addition, combi-score results to be even more precise and further suggests that these combinations can be used as a refined version of the 30-genes NK whole-signature.
...
Generally speaking since the higher is the length of random signatures, the higher will be the noise hence the probability to pick genes expressed in all the clusters: this suggests that __the higher is the length of a real DEG signature the higher is the probability to include less specific genes and to increase the noise__.
