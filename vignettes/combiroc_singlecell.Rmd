...
The computationally heavy passages are therefore marked as [[Heavy-Code]]{style="color: red;"} and can be skipped by the user by reading/loading **pre-computed objects or intermediate results either from `rds` or `csv` files**.
...
The difficulty to distinguish NK (Natural Killer) cells from CD8+ T cells in many scRNAseq datasets is a good challenge to test combiroc performance in markers signature optimization; after performing differential marker expression we took into account the top 30 differentially expressed genes (DEGs) specific for NK cells cluster.
We are going to use combiroc to select the best combinations of markers among these top 30 NK cells markers.
Regression models from the selected combinations will then be used on other independent datasets of the same kind (test/validation datasets).

**TRAINING DATASET**

As combiroc training dataset we used the [PBMC CITE-seq atlas](https://satijalab.org/seurat/articles/multimodal_reference_mapping.html) from Hao et al. 2021 [[2]](https://www.sciencedirect.com/science/article/pii/S0092867421005833).
This dataset can be downloaded as an h5 Seurat data (h5s) from the [FredHutch-NYCG atlas page](https://atlas.fredhutch.org/nygc/multimodal-pbmc/) (exact download link: [here](https://atlas.fredhutch.org/data/nygc/multimodal/pbmc_multimodal.h5seurat)) [[Heavy-Code]]{style="color: red;"}
...
![](../inst/precomp/images/atlas_dimplot.png){width="80%"}

To guarantee the best possible performance of the training dataset (the one we extract the markers combinations from) is important to make sure its value ranges are similar to those of the test/validation datasets.
For this reason we advise using the raw counts matrices for all the datasets, in order to be able to consistently rescale them in the same way by using the function `Seurat::ScaleData()`.

## Extracting the NK markers

The gene markers for NK cells were obtained with a standard Seurat analysis (see [here](https://satijalab.org/seurat/articles/pbmc3k_tutorial.html)); the `Seurat::FindMarkers()` function was used, then markers were ordered by fold change: [[Heavy-Code]]{style="color: red;"}
...
## Converting Seurat object in combiroc ready data

Central to the combiroc workflow applied to single cell data is the function `seurat_to_combiroc()`.
It takes a Seurat object as input and extracts both the selected marker expression values from the @data slot of a given assay and, optionally, the class of any specific celltype (in our case 'NK' or 'Other'); then it directly assembles a dataframe compatible with combiroc workflow either for training (finding combinations and models) or test (using combinations) purposes.

## Building the training dataset

Once you have a Seurat object obtained from a standard Seurat protocol, it needs to be converted in a combiroc-ready version in order to perform the training procedure (i.e. finding the markers combinations and models).
Such a `train` object can be obtained with the `seurat_to_combiroc()` function: [[Heavy-Code]]{style="color: red;"}
...
The `train` object obtained with the code above can be directly read here:
...
Then, as described in the [main combiroc vignette](https://ingmbioinfo.github.io/combiroc/articles/combiroc_standard.html), we used `combiroc_long` to make the data in long form, fit for further processing:
...
The `combi()` function works on the `train` dataset and it computes the marker combinations and counts their corresponding positive samples for each class (once thresholds are selected).
...
As described in the [combiroc's vignette for the standard workflow](https://ingmbioinfo.github.io/combiroc/articles/combiroc_standard.htm), the argument **signalthr** in the `combi()` function should be set depending on the guidelines and characteristics of the methodology used for the analysis or by an accurate inspection of signal intensity distribution.
In case of lack of specific guidelines, one should set the value signalthr as suggested by the `distr$Density_plot` feature.
...
To obtain a performant and stringent signature refinement we set `min_SP =80` and the highest possible `min_SP =78` in `marker_distribution()`.
In `combi()`, we raised `combithr` argument to **2**: this is the minimum number of positively expressed genes (i.e. the minimum number of genes that need to reach the signal threshold) to consider the whole combination as a valid one.
...
```{r}
autoThreshold <- median(distr$Coord$threshold)
autoThreshold
```
...
[[Heavy-Code]]{style="color: red;"} *(use the pre-computed below)*
...
While the cell clusters annotation of the first (training) dataset was indeed used in the computations by combiroc (we needed to distinguish NK cells from all other cells), the cells annotation of the other two validation datasets is stripped away from the data, and **it will be only used *ex-post* as a ground truth to check the expression of maker combinations**.
...
The dataset hah been generated by Stoeckius et al. 2017 [[3]](https://www.nature.com/articles/nmeth.4380).
...
The PBMC3K test dataset can also be directly invoked from the `SeuratData` library.
The dataset is immediately available as a Seurat object named `pbmc3k.final`
...
In the same way as oreviously done we are using now `seurat_to_combiroc()` to obtain the second combiroc test dataset (`test_pbmc3k`).
Please note that this dataset does not contain 'TRDC expression values since this gene ' (which is part of the selected combinations) is not present in the original PBMC3K dataset.
...
## Distribution of top single markers

Let's have a look how the top four single NK-markers expression is distributed in the CBMC dataset:

```{r warning=FALSE, message=FALSE, fig.height = 10, fig.width = 8, fig.align = "center"}
p1 <- FeaturePlot(cbmc, features = nk_genes[1:4])
p2 <- VlnPlot(cbmc, features = nk_genes[1:4], group.by = 'ID', pt.size=0, ncol = 2)
p1
p2
```

In CBMC dataset, single markers values are higher in NK cluster and moderate in all the others, but still slightly present in some CD8 T cells, CD34+ and CD16+ Mono cells, even if clusters have been defined by the authors relying on protein-level (CITE seq) information.
Moreover, GZMB per se is not sufficient to discriminate NK cells from pDC.
In this dataset only PRF1 among the top four markers is able to discriminate NK cells alone.
Mouse cells (the "NA" cluster), which are kept from the original dataset as negative control, also show a bit of GNLY and NKG7 expression.

And the same in the PBMC3K dataset:

```{r warning=FALSE, message=FALSE, fig.height = 10, fig.width = 8, fig.align = "center"}
p3 <- FeaturePlot(pbmc3k.final, features = nk_genes[1:4])
p4 <- VlnPlot(pbmc3k.final, features = nk_genes[1:4], group.by = 'ID', pt.size=0, ncol = 2)
p3
p4
```

In PBMC3K dataset, all top four markers have high expression values on CD8 T cells too, showing that single markers mRNA level may be not sufficient to clearly identify the cells of interest.

## Distributions of the best combination

We now look at the performance of the selected marker combinations.
To assess the descriptive power, for both test datasets, we compare the expression of top 4 single markers with the expression of the best combination (since the top 4 are very similar in terms of performance), exploiting both our "combi-score" and an already published "gene-signature-score".

While the **combi-score** defined here is defined as the predicted probability obtained while fitting the combinations models to the test datasets (returned by `combi_score()`), the **gene-signature-score** that we use as a comparison is described in Della Chiara et al. 2021 [[5]](https://www.nature.com/articles/s41467-021-22544-y): it takes into account both the expression level and co-expression of genes within each single cell.
...
We reproduced this score in the custom R function `signature_score.R`:

```{r}
source("inst/external_code/signature_score.R")
```

First, we compute the "gene-signature-score" of the **whole 30 DEGs signature** that we assume as reference.
...
We then calculate both the "gene-signature-score" and the "combi-score" of the top combinations: - *Combination 172173* (made by genes GZMB, IL2RB, KLRF1, SPON2 and TRDC), with the highest accuracy, is investigated in CBMC dataset.
- *Combination 137550* (made by genes CLIC3, FCGR3A, IL2RB, KLRD1 and KLRF1) is used in PBMC3K dataset since it's the first top combination without the TRDC gene, which is not expressed in the this dataset.
...
Strikingly, in CBMC the highest scores are all found in NK cluster, while in all the others the values are drastically lower and all comparable to mouse cell ones (negative control).
In PBMC3K the highest scores are in NK cells but a still relevant score is found in cytotoxic CD8-T cells.
The signal of the whole-30genes-signature is, as expected, less noisy than the signal from the single top markers.
...
In PBMC3K, gene score values in CD8 T cells were moderately higher and closer to NK cluster ones with respect to combiroc score values.
This difference was expected since to compute the gene scores the genes within the combinations are considered equally, while combiroc models assign a 'weight' to each gene, further indicating the power of combiroc combination models in suggesting the identity of a cluster with a modest amount of genes.
...
In fact, random scores show quite homogeneously distributed values across clusters indicating that the cluster-preferential distributions of real combinations scores are not stochastic.
