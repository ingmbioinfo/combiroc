...
vignette: |
  %\VignetteIndexEntry{Quick guide to CombiROC} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
All these plotting functions take as input the `tab` dataframe obtained with the `combi()` function (see above) and containing the SE and SP values for cases and controls in all marker combinations. The original `tab` dataframe can be customly filtered to plot only a subset of combinations meeting specific criteria.  
...
tab %>% 
  ggplot(aes(x = `SP`, y = `SE`, size = `n_markers` )) +
...
tab %>%
  mutate(Combo = ifelse((`SP`>min_SP & `SE`>min_SE),"gold","below_thr")) %>% 
  ggplot(aes(x = `SP`, y = `SE`, size = `n_markers`, color = Combo)) +
